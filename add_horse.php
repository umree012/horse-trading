...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($gender) || empty($height) || empty($color) || empty($breed) || empty($age) || empty($price) || empty($location)) {
        ...
        if (empty($error)) {
            ...
            $stmt = $pdo->prepare("INSERT INTO horses (seller_id, name, gender, height, color, breed, age, price, location, description, image_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if ($stmt->execute([$seller_id, $name, $gender, $height, $color, $breed, $age, $price, $location, $description, $image_path])) {
                $success = 'Horse added successfully!';
                // Clear form
                $_POST = array();
            } else {
                $error = 'Failed to add horse. Please try again.';
            }
        }
    }
}
...
<head>
    ...
    <title>Add Horse - Horse Trading</title>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="add_horse.php">Add Horse</a></li>
                </ul>
                <ul class="navbar-nav">
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header bg-primary text-white">
                        <h2 class="text-center">Add New Horse</h2>
                    </div>
                    <div class="card-body">
                        <?php if ($error): ?>
                        ...
                        <?php endif; ?>
                        <form method="POST" action="add_horse.php" enctype="multipart/form-data">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="gender" name="gender" required>
                                        ...
                                        <option value="gelding" <?= ($_POST['gender'] ?? '') === 'gelding' ? 'selected' : '' ?>>Gelding</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="number" step="0.1" class="form-control" id="height" name="height" value="<?= htmlspecialchars($_POST['height'] ?? '') ?>" required>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label for="color" class="form-label">Color</label>
                                    <input type="text" class="form-control" id="color" name="color" value="<?= htmlspecialchars($_POST['color'] ?? '') ?>" required>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label for="breed" class="form-label">Breed</label>
                                    <input type="text" class="form-control" id="breed" name="breed" value="<?= htmlspecialchars($_POST['breed'] ?? '') ?>" required>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    <label for="age" class="form-label">Age (years)</label>
                                    <input type="number" class="form-control" id="age" name="age" value="<?= htmlspecialchars($_POST['age'] ?? '') ?>" required>
                                </div>
                                <div class="col-md-4 mb-3">
                            ...
                            <button type="submit" class="btn btn-primary w-100">Add Horse</button>
                        </form>
                    </div>
                </div>
            </div>
